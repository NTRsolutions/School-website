			...
			<ul class="nav navbar-nav side-nav">
                ...
                <li><a href="admin.php"><i class="fa fa-fw fa-home"></i> Home</a></li>
                 <li>
                    ...
                    <li class="active"><a href="javascript:;" data-toggle="collapse" data-target="#subject"><i class="fa fa-fw fa-book fa-fw"></i> Subjects<i class="fa fa-fw fa-caret-down"></i></a>
                                ...
                                <tbody>
                                    ...
                                    <?php
                                        ...
                                        <tr>
                                            ...
                                            <?php $i++; ?>  
                                            <td style="padding:10px"><input id="checkbox[]" name="checkbox[]" style="width: 15px;height: 15px" type="checkbox" value="<?php echo $rows['id']; ?>" ></td>
                                            <td><?php echo $rows['name']; ?></td>
                                            <?php
                                                $x=$rows['name'];
                                                $query1="SELECT * from teacher WHERE subject1='$x' OR subject2='$x' OR subject3='$x' OR subject4='$x' OR subject5='$x'";
                            ...
                            <?php
                                if(isset($_POST['delete']))
                                {
                                    if(@$checkbox = $_POST['checkbox'])
                                    {
                                      ...
                                      {
                                        $del_id = $checkbox[$i];
                                        $sql = "SELECT * FROM subject WHERE id='$del_id'";
                                        $result = mysqli_query($stat,$sql);
                                        if($rows=mysqli_fetch_array($result))
                                        {
                                            $na=$rows['name'];
                                        }
                                        $sql = "DELETE FROM subject WHERE id='$del_id'";
                                        $result = mysqli_query($stat,$sql);
                                        for ($j=1; $j <=5 ; $j++) 
                                        { 
                                            $sql = "UPDATE teacher SET subject".$j."='' WHERE subject".$j."='$na'";
                                            $result = mysqli_query($stat,$sql); 
                                        }
                                        $sql = "UPDATE class SET subject='',teacher='' WHERE subject='$na'";
                                        $result = mysqli_query($stat,$sql); 
                                        for ($j=1; $j <=9 ; $j++) 
                                        { 
                                            $sql = "UPDATE class SET subject".$j."='',teacher".$j."='' WHERE subject".$j."='$na'";
                                            $result = mysqli_query($stat,$sql); 
                                        }
                                      }
                                      if($result)
                                      {
                                        ...
                                        echo "<meta http-equiv=\"refresh\" content=\"0;URL=admin_delsubject.php\">";
                                        }                   
                                    }
                                    else
                                    {
                                      ?>
                                            <script>
                                            document.getElementById("incor1").innerHTML="Please select atleast one checkbox!!!";
                                            </script>
                                            <?php 
                                    }
                                }
