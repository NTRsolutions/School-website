...
if(isset($_POST['submit']))
{
  ...
  if((strcmp($type,"Quarterly examination")!=0)&&(strcmp($type,"Half yearly examination")!=0)&&(strcmp($type,"Annual examination")!=0))
  {
  $name_test=$type;
  $temp_flag=1;
  }
  ...
  if($temp_flag==1)
    $query="SELECT id FROM marks WHERE std='$std_t' && subj='$subj' && name='$name_test'";
  else
  $query="SELECT id FROM marks WHERE std='$std_t' && subj='$subj' && type='$type'";
  ...
  while($row=mysqli_fetch_array($result))
  {
	...
	$execute_once=1;
	 ...
	 if(!strcmp(trim($mark),""))
	   $mark="Absent";
     $query="UPDATE marks SET marks='$mark' WHERE id='$id'";
	 mysqli_query($stat,$query);
	 $query="UPDATE marks SET max='$max' WHERE id='$id'";
	 mysqli_query($stat,$query);
	 $flag=1;
  }
        ...
        <div id="page-wrapper">
            ...
            <div class="container-fluid">
				...
				<div class="alert alert-danger">
				  <strong>Maximum marks cannot be left empty.</strong>
				 </div>
				 <?php
				 }
				 ?>
			...
			while($row=mysqli_fetch_array($result))
			   ...
			   while($l<$i)
			   {
				...
				<div class="alert alert-info">
				  <strong>An empty "Marks awarded" field denotes an absentee</strong>
				 </div>
				<!-- /.row -->
             ...
             <?php
				...
				?>
				  ...
				  <?php
					...
					<input type="number" name="<?php echo $row['id'];?>" value="<?php if(!strcmp($row['marks'],"Absent"))
					echo "";
					else
					echo $row['marks'];?>" style="width:150px;" class="form-control" id="<?php echo "reg_".$row['regno'];?>" type="text" />
				   ...
				   <input type="number" name="<?php echo "max_".$sample_id;?>" style="width:75px;" class="form-control" value="<?php echo $max_mark;?>"/><br/>
				   
				   <button name="submit" id="submit" type="submit" class="btn btn-default">Edit marks</button>
